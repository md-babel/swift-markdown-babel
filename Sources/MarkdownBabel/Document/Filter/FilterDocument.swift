...
where Base: Document, Element: Markdown.Markup {
	...
	public func markdown(visitor: @escaping (AnyElement) -> AnyElement?) -> Base.VisitedDocument {
		return base.markdown(visitor: { element in
			guard let element = element as? Element, predicate(element)
			else {
...
extension Document {
	...
	public func filter<Element>(_ predicate: @escaping (Element) -> Bool) -> FilterDocument<Self, Element>
	where Element: Markdown.Markup {
		return FilterDocument(base: self, predicate)
	}
}
